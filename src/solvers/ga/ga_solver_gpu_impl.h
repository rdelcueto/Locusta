namespace locusta {
    ...
    template <typename TFloat>
        void ga_solver_gpu<TFloat>::_initialize_population()
    {
        TFloat * const pop_data = _dev_population->_get_dev_transformed_data_array();
        const uint32_t pop_size =
            _dev_population->_NUM_DIMENSIONS *
            _dev_population->_NUM_AGENTS *
            _dev_population->_NUM_ISLES;

        _bulk_prn_generator->_generate(pop_size, pop_data);
        _dev_population->_initialize();
    }
