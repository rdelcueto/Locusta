...
namespace locusta {
  ...
  template<typename TFloat>
  void de_solver_cpu<TFloat>::setup_solver() {
  ...
  template<typename TFloat>
  void de_solver_cpu<TFloat>::teardown_solver() {
    delete [] _prn_sets;
    delete [] _bulk_prns;
  }

  template<typename TFloat>
  void de_solver_cpu<TFloat>::setup_operators(DeBreedFunctor<TFloat> * breed_functor_ptr,
  ...
  template<typename TFloat>
  void de_solver_cpu<TFloat>::solver_config(uint32_t migration_step,
  ...
  template<typename TFloat>
  void de_solver_cpu<TFloat>::advance() {
  ...
  template<typename TFloat>
  void de_solver_cpu<TFloat>::transform() {
  ...
  template<typename TFloat>
  void de_solver_cpu<TFloat>::trial_vector_replace() {
    ...
    for(uint32_t i = 0; i < _ISLES; i++) {
      for(uint32_t j = 0; j < _AGENTS; j++) {
        ...
        if (trial_fitness[FITNESS_OFFSET] > previous_fitness[FITNESS_OFFSET]) {
          ...
          const uint32_t GENOME_OFFSET = i * _AGENTS * _DIMENSIONS + j * _DIMENSIONS;
