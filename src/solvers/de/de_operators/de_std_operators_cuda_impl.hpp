...
namespace locusta {
  ...
  struct DeWholeCrossoverCuda : DeBreedCudaFunctor<TFloat> {
    ...
    void operator()(de_solver_cuda<TFloat> * solver) {
      ...
      const TFloat * prn_array = const_cast<TFloat *>(solver->_prn_sets[de_solver_cuda<TFloat>::BREEDING_SET]);
      ...
      const uint32_t * trial_selection = const_cast<uint32_t *>(solver->_dev_recombination_idx_array);

      const TFloat * current_vectors = const_cast<TFloat *>(solver->_dev_population->_dev_data_array);

      TFloat * trial_vectors = solver->_dev_population->_dev_transformed_data_array;
  ...
  struct DeRandomSelectionCuda : DeSelectionCudaFunctor<TFloat> {
    ...
    void operator()(de_solver_cuda<TFloat> * solver) {
      ...
      const TFloat * prn_array = const_cast<TFloat *>(solver->_prn_sets[de_solver_cuda<TFloat>::SELECTION_SET]);

      uint32_t * recombination_idx_array = solver->_dev_recombination_idx_array;
      uint32_t * recombination_reservoir_array = solver->_dev_recombination_reservoir_array;
