...
namespace locusta {
  ...
  de_solver_cuda<TFloat>::de_solver_cuda(population_set_cuda<TFloat> * population,
    ...
    CudaSafeCall(cudaMalloc((void **) &(_dev_previous_fitness_array), TOTAL_AGENTS * sizeof(TFloat)));
    CudaSafeCall(cudaMalloc((void **) &(_dev_recombination_idx_array), TOTAL_AGENTS * sizeof(uint32_t)));
    CudaSafeCall(cudaMalloc((void **) &(_dev_recombination_reservoir_array), _ISLES * _AGENTS * _AGENTS * sizeof(uint32_t)));
  }
  ...
  template<typename TFloat>
  void de_solver_cuda<TFloat>::setup_solver() {
  ...
  template<typename TFloat>
  void de_solver_cuda<TFloat>::teardown_solver() {
    delete [] _prn_sets;
    CudaSafeCall(cudaFree(_dev_bulk_prns));
  }

  template<typename TFloat>
  void de_solver_cuda<TFloat>::setup_operators(DeBreedCudaFunctor<TFloat> * breed_functor_ptr,
  ...
  template<typename TFloat>
  void de_solver_cuda<TFloat>::solver_config(uint32_t migration_step,
  ...
  template<typename TFloat>
  void de_solver_cuda<TFloat>::advance() {
  ...
  template<typename TFloat>
  void de_solver_cuda<TFloat>::transform() {
  ...
  template<typename TFloat>
  void de_solver_cuda<TFloat>::trial_vector_replace() {
