        ...
        void operator()(pso_solver_cpu<TFloat> * solver)
            {
                ...
                const TFloat * position_array = const_cast<TFloat *>(solver->_population->_data_array);
                    ...
                    for(uint32_t j = 0; j < AGENTS; j++) {
                        ...
                        if (curr_fitness > candidate_fitness) {
                            ...
                            for(uint32_t k = 0; k < DIMENSIONS; k++) {
                                ...
                                position_record[curr_gene] =
                                    position_array[curr_gene];
                            }
                        }
                   }
                }
        ...
        void operator()(pso_solver_cpu<TFloat> * solver)
            {
               ...
               // Solver configuration constants
                ...
                const TFloat * positions = const_cast<TFloat *>(solver->_population->_data_array); // Current
                                                                                      // particle position.
                const TFloat * position_record = const_cast<TFloat *>(solver->_cognitive_position_vector); // (Cognitive)
                                                                                       // Particle's
                                                                                       // records
                const TFloat * isle_position_record = const_cast<TFloat *>(solver->_max_agent_genome); // (Social)
                                                                     // Isle's
                                                                     // records
