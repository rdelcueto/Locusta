...
template <typename TFloat>
void
update_records_dispatch(const uint32_t ISLES, const uint32_t AGENTS,
                        const uint32_t DIMENSIONS, const TFloat* data_array,
                        const TFloat* fitness_array, TFloat* max_agent_genome,
                        TFloat* min_agent_genome, TFloat* max_agent_fitness,
                        TFloat* min_agent_fitness)
{
  update_records_kernel<<<ISLES, AGENTS,
                          2 * AGENTS*(sizeof(uint32_t) + sizeof(TFloat))>>>(
    DIMENSIONS, data_array, fitness_array, max_agent_genome, min_agent_genome,
    max_agent_fitness, min_agent_fitness);

  CudaCheckError();
}

template void update_records_dispatch<float>(
  const uint32_t ISLES, const uint32_t AGENTS, const uint32_t DIMENSIONS,
  const float* data_array, const float* fitness_array, float* max_agent_genome,
  float* min_agent_genome, float* max_agent_fitness, float* min_agent_fitness);

template void update_records_dispatch<double>(
  const uint32_t ISLES, const uint32_t AGENTS, const uint32_t DIMENSIONS,
  const double* data_array, const double* fitness_array,
  double* max_agent_genome, double* min_agent_genome, double* max_agent_fitness,
...
template <typename TFloat>
void
initialize_vector_dispatch(const uint32_t ISLES, const uint32_t AGENTS,
                           const uint32_t DIMENSIONS,
                           const TFloat* LOWER_BOUNDS, const TFloat* VAR_RANGES,
                           const TFloat* tmp_vec, TFloat* dst_vec)
{
  initialize_vector_kernel<<<ISLES, AGENTS>>>(DIMENSIONS, LOWER_BOUNDS,
                                              VAR_RANGES, tmp_vec, dst_vec);
  CudaCheckError();
}

// Template initializer (float)
template void initialize_vector_dispatch<float>(
  const uint32_t ISLES, const uint32_t AGENTS, const uint32_t DIMENSIONS,
  const float* LOWER_BOUNDS, const float* VAR_RANGES, const float* tmp_vec,
  float* dst_vec);

// Template initializer (double)
template void initialize_vector_dispatch<double>(
  const uint32_t ISLES, const uint32_t AGENTS, const uint32_t DIMENSIONS,
  const double* LOWER_BOUNDS, const double* VAR_RANGES, const double* tmp_vec,
  double* dst_vec);
...
template <typename TFloat>
void
crop_vector_dispatch(const uint32_t ISLES, const uint32_t AGENTS,
                     const uint32_t DIMENSIONS, const TFloat* UPPER_BOUNDS,
                     const TFloat* LOWER_BOUNDS, TFloat* vec)
{
  crop_vector_kernel<<<ISLES, AGENTS>>>(DIMENSIONS, UPPER_BOUNDS, LOWER_BOUNDS,
                                        vec);
  CudaCheckError();
}

// Template initializer (float)
template void crop_vector_dispatch<float>(
  const uint32_t ISLES, const uint32_t AGENTS, const uint32_t DIMENSIONS,
  const float* UPPER_BOUNDS, const float* LOWER_BOUNDS, float* vec);

// Template initializer (double)
template void crop_vector_dispatch<double>(
  const uint32_t ISLES, const uint32_t AGENTS, const uint32_t DIMENSIONS,
  const double* UPPER_BOUNDS, const double* LOWER_BOUNDS, double* vec);
}
